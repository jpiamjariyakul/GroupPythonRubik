...
import cubeGenerate.display as cubeDisplay
...
def main():
	...
	for face in range(0, 3): # Runs for camera affiliated with URF faces
		...
		for row in range(3):	# Loop to fill all rows/columns
			...
			for column in range(3):
				cubelets_0[face][row].append(colorCheck.verifyColor(	0, face, row, column, result_combined[0],
																result_color[0][0], result_color[0][1], result_color[0][2],
																result_color[0][3], result_color[0][4], result_color[0][5]
																))
	for face in range(0, 3): # Runs for camera affiliated with DLB faces
		...
		for row in range(3):	# Loop to fill all rows/columns
			...
			for column in range(3):
				cubelets_1[face][row].append(colorCheck.verifyColor(	1, face, row, column, result_combined[1],
																result_color[1][0], result_color[1][1], result_color[1][2],
																result_color[1][3], result_color[1][4], result_color[1][5]
																))
