...
from var_def import coord_yx
from var_def import dict_faceColor

# Outputs frames of different colors, including raw (scaled) & combined images
def colorFrames(frame, vals_hsv):
	...
	hsv_colorRange = (	(np.array(vals_hsv[0][0]),	np.array(vals_hsv[0][1])), 	# Bound of white
						(np.array(vals_hsv[1][0]),	np.array(vals_hsv[1][1])),	# Bound of red_low
						(np.array(vals_hsv[3][0]), 	np.array(vals_hsv[3][1])),	# Bound of orange
						(np.array(vals_hsv[4][0]), 	np.array(vals_hsv[4][1])),	# Bound of yellow
						(np.array(vals_hsv[5][0]), 	np.array(vals_hsv[5][1])),	# Bound of green
						(np.array(vals_hsv[6][0]),	np.array(vals_hsv[6][1])))	# Bound of blue
	hsv_redHigh = (np.array(vals_hsv[2][0]),	np.array(vals_hsv[2][1]))	# Bound of red_high - special case!
	#print(hsv_colorRange)
...
def readFrame(cap, camera, vals_hsv):
	ret, frame = cap.read()
	#frame = cv2.resize(frame, (300, 300))
	result_raw, result_combined, result_color = colorFrames(frame, vals_hsv)
...
### TO DAN:	Change the number in cv2.VideoCapture() to change the cameras
# 	IE	0 = built-in webcam facing you
#		1 = built-in webcam facing away, OR first camera connected
#		2 = first camera connected (assuming two built-in cams)	...
#		Mess around w/ the values & see what happens!
def cam_initCap():
	cap_0 = cv2.VideoCapture(1, cv2.CAP_DSHOW) # Camera 1
	cap_1 = cv2.VideoCapture(2, cv2.CAP_DSHOW) # Camera 2
	return cap_0, cap_1

def cam_obtain(cap_0, cap_1, vals_hsv):
	result_raw_0, result_combined_0, result_color_0 = readFrame(cap_0, 0, vals_hsv)
	result_raw_1, result_combined_1, result_color_1 = readFrame(cap_1, 1, vals_hsv)
	'''
